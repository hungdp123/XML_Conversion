...
def convert_to_WOVOdat_format(events):
    ...
    for event in events:
        ...
        if (event['origin']['timeUncertainty'] != 'None'):
            originTimeUnc = ET.SubElement(networkEvent, 'originTimeUnc')
        ...
        networkEvent.attrib = {'code': "ISC_" + event['eventInfo']['publicID'][-9:] + "_" + convert_time_to_code(event['origin']['time']), 'network': "ISC_seisnet",
                               'owner1': 'ISC', 'pubDate': '2020-12-31 00:00:00'}
        ...
        if (event['origin']['timeUncertainty'] != 'None'):
            originTimeUnc.text = event['origin']['timeUncertainty']
